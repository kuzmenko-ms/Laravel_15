...
<div class="bx_size"><ul id="#ITEM#_prop_<? echo $arProp['ID']; ?>_list" ><?
...
<div class="bx_scu"><ul id="#ITEM#_prop_<? echo $arProp['ID']; ?>_list" style="width: <? echo $strWidth; ?>;"><?
				foreach ($arProp['VALUES'] as $arOneValue)
				{
					?><li
						data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID'] ?>"
...
<div class="bx_catalog_list_home_new">
     
      <div class="item_metro">                                                                     
            <div class="t">                                                                             
              <img src="<?=(SITE_TEMPLATE_PATH)?>/images/new/1.jpg">                                                                     
              <div class="desc">                                                                          
                <a href="#" class="ti">ЗАКАЗАТЬ АВТОСТЕКЛО</a>                         Отправьте заявку и наши специалисты подберут стекло и выполнят заказ в ближайшее время                                                                     
              </div>                                                                 
            </div>                                                                   
            <div class="t1">                                                                               
              <img style="float:left;" src="<?=(SITE_TEMPLATE_PATH)?>/images/new/3.jpg">                                                 
              <a href="#" class="ti">ЗАПИСАТЬСЯ НА УСТАНОВКУ  </a>                                                     
              <div class="min_desc">Отправьте заявку и мы зарезервируем удобное время!                                                                  
              </div>                                                            
            </div> 
              <div class="t_in">                                                                             
              <img src="<?=(SITE_TEMPLATE_PATH)?>/images/new/23.jpg">                                                                     
              <div class="desc">                                                                          
                <a href="#" class="ti">ПОДБОР АВТОСТЕКЛ</a>Подбор автостекла по марке автомобиля                                                        
              </div>                                                                 
            </div>                                                        
          </div>
...
<div class="bx_catalog_list_home col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>">
     ...
     <img class="bg_img_category" src="<?=$arResult["PICTURE"]["SRC"]?>" alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>" /> 
     
          
  <div id="div2" class="cont_test"></div>
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
  ...
  <div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
	...
	?>
         ...
         echo (0 < $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT']; ?>%</div>
...
<?
	...
	else
	{
		...
		<div style="background:#8ac5e3;right:0px; height:44px; text-align:right;" class="bx_catalog_item_controls_blocktwo">
         <span class="namee" >
       
   
        <? echo $arItem["PROPERTIES"]["ATT_TYPE"]["VALUE"];   ?>
                             </span>

			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" style="border-left: 1px solid white;" class="bx_bt_button bx_medium" href="javascript:void(0)" rel="nofollow"><?
			echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
			?></a>
		</div>
...
?>
  	...
  	<?foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
        ...
        if($arOneProp['DISPLAY_VALUE']=="заднее"){
        
        ?>
        
        	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style="float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/zadnee.jpg">  </a>
        <?}
         ...
         if($arOneProp['DISPLAY_VALUE']=="лобовое"){         ?>
      	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img  class="t_im" style="float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/lobovoe.jpg">  </a>
        ...
        <?} ?>
           ...
           <?if($arOneProp['DISPLAY_VALUE']=="боковое задняя левая дверь"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/bokovoe_lev_zad.jpg">  </a>
        
        <?}?>
        
        
                <?if($arOneProp['DISPLAY_VALUE']=="передняя левая дверь"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/bokovoe_lev_pered.jpg">  </a>
        
        <?}?>
        
        
            <?if($arOneProp['DISPLAY_VALUE']=="передняя правая дверь"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/bokovoe_prav_pered.jpg">  </a>
        
        <?}?>
        
      
           <?if($arOneProp['DISPLAY_VALUE']=="задняя правая дверь"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/bokovoe_prav_zad.jpg">  </a>
        
        <?}?>
        
      
      
         <?if($arOneProp['DISPLAY_VALUE']=="форточка задней правой двери"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im" style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/zad_f_lev.jpg">  </a>
        
        <?}?>
        
              <?if($arOneProp['DISPLAY_VALUE']=="форточка задней левой двери"){?>
              	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">  <img class="t_im"  style=" float:left"  src="<?=(SITE_TEMPLATE_PATH)?>/images/glass/zad_f_prav.jpg">  </a>
...
<?
			...
			if ($boolShowProductProps)
			{
				foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
				{
				?><tr><td><strong><? echo $arOneProp['NAME']; ?></strong></td> <td><?
					echo (
						is_array($arOneProp['DISPLAY_VALUE'])
